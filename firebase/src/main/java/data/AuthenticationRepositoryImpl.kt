...
class AuthenticationRepositoryImpl @Inject constructor(
    ...
    override suspend fun signUp(email: String, password: String): domain.event.SignUpEvent {
        return try {
            ...
            if(isUserAlreadyCreated.isNullOrEmpty()){
                domain.event.SignUpEvent.Error(error = "User already exists")
            }
            firebaseAuth.createUserWithEmailAndPassword(email, password).await()
            return domain.event.SignUpEvent.Success
        }catch(e:Exception){
            domain.event.SignUpEvent.Error(error = e.message ?: "Unknown error")
        }
    }
    ...
    override suspend fun login(email: String, password: String): domain.event.SignInEvent {
        return try {
            domain.event.SignInEvent.Loading
            delay(20)
            domain.event.SignInEvent.Success(
                result = firebaseAuth.signInWithEmailAndPassword(email, password).await()
            )
        }catch (e: FirebaseAuthInvalidUserException){
            domain.event.SignInEvent.Error(error = "Invalid email or user does not exist")
        }catch (e: FirebaseAuthInvalidCredentialsException){
            domain.event.SignInEvent.Error(error = "Invalid data")
        }
        catch (e:Exception){
            domain.event.SignInEvent.Error(e.message ?: "Unknown error")
        }
    }
    ...
    override suspend fun reloadUser(){
        try {
            ...
            domain.event.ReloadingEvent.Error(message = "${e.message}")
        }
    }
